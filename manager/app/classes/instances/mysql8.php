...
return (new class() extends Instance
{
		...
		Installer::form_begin($this->package);
			...
			Layout::tab_panes_begin();
				...
				Layout::tab_pane_begin(2);
					...
					Installer::form_text(["id"=>"tcp_port_x", "label"=>"TCP Port (X plugin)", "placeholder"=>"The TCP port to listen on for incoming connections for X plugin", "value"=>33060]);
	...
	public function verify()
	{
		...
		$tcp_port_x  = $_POST['tcp_port_x'];

		$v->control('tcp_port')->number();
		$v->control('tcp_port_x')->number();

		if (Util::endpointInUse('tcp', $interface, $tcp_port))
			$v->control('tcp_port')->custom('Port already assigned or in use.');

		if (Util::endpointInUse('tcp', $interface, $tcp_port_x))
			$v->control('tcp_port_x')->custom('Port already assigned or in use.');

		if ($tcp_port == $tcp_port_x)
			$v->control('tcp_port_x')->custom("Port can't be the same as main TCP PORT.");
	...
	public function install()
	{
		...
		$tcp_port_x = $_SESSION['pdata']['tcp_port_x'];
		$this->registerEndpoint("tcp", $interface, $tcp_port, "mysql");
		$this->registerEndpoint("tcp", $interface, $tcp_port_x, "mysql_x");
		...
		$random_port_x = Util::getFreeLocalTCPPort([$random_port]);
		...
		$data .= 'port='.$random_port.N;
		$data .= 'mysqlx-bind-address=127.0.0.1'.N;
		$data .= 'mysqlx-port='.$random_port_x.N;
		...
		$data .= 'max_allowed_packet='.$_SESSION['pdata']['max_allowed_packet'].'M'.N;
		$data .= 'default-authentication-plugin=mysql_native_password';
		...
		if (!empty($_SESSION['pdata']['root_password']))
		{
			...
			$root_password_query = "IDENTIFIED WITH mysql_native_password BY '$root_password'";
		}
		else
		{
			$root_password = null;
		...
		if ($final_interface != '127.0.0.1')
		{
			...
			$query .= "CREATE USER `root`@`$final_interface` $root_password_query;";
			$query .= "GRANT ALL PRIVILEGES ON *.* TO `root`@`$final_interface` WITH GRANT OPTION;";
		}
		else
		{
		...
		if ($root_password)
		{
			$query .= "ALTER USER `root`@`localhost` $root_password_query;";
		...
		$data .= 'port='.$tcp_port.N;
		$data .= 'mysqlx-bind-address='.$interface.N;
		$data .= 'mysqlx-port='.$tcp_port_x.N;
		$data .= 'datadir='.slash($datadir).N;
		$data .= 'log-error='.slash($this->path).'/error.log'.N;
		$data .= 'pid-file='.slash($this->path).'/mysqld.pid'.N;
		$data .= 'key_buffer_size='.$_SESSION['pdata']['key_buffer_size'].'M'.N;
