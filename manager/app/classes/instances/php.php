...
return (new class() extends Instance
{
		...
		Installer::form_begin($this->package);
			...
			Layout::tab_panes_begin();
				...
				Layout::tab_pane_begin(4);
					...
					Installer::form_checkbox(["id"=>"php_com_dotnet",   "label"=>"com_dotnet",   "checked"=>false]);
					Installer::form_checkbox(["id"=>"php_curl",         "label"=>"curl",         "checked"=>true]);
					Installer::form_checkbox(["id"=>"php_enchant",      "label"=>"enchant",      "checked"=>false]);
					Installer::form_checkbox(["id"=>"php_fileinfo",     "label"=>"fileinfo",     "checked"=>true]);
					Installer::form_checkbox(["id"=>"php_gd2",          "label"=>"gd2",          "checked"=>true]);
					...
					Installer::form_checkbox(["id"=>"php_interbase",    "label"=>"interbase",    "checked"=>false]);
					...
					if ($this->package['version'] <= '5.6.30')
						Installer::form_checkbox(["id"=>"php_mysql",        "label"=>"mysql",        "checked"=>false]);

					Installer::form_checkbox(["id"=>"php_mysqli",       "label"=>"mysqli",       "checked"=>true]);
					Installer::form_checkbox(["id"=>"php_oci8_12c",     "label"=>"oci8_12c",     "checked"=>false]);
					...
					Installer::form_checkbox(["id"=>"php_pdo_sqlite",   "label"=>"pdo_sqlite",   "checked"=>false]);
					...
					Installer::form_checkbox(["id"=>"php_sybase_ct",    "label"=>"sybase_ct",    "checked"=>false]);
					Installer::form_checkbox(["id"=>"php_tidy",         "label"=>"tidy",         "checked"=>false]);
					Installer::form_checkbox(["id"=>"php_xmlrpc",       "label"=>"xmlrpc",       "checked"=>false]);
					Installer::form_checkbox(["id"=>"php_xsl",          "label"=>"xsl",          "checked"=>false]);

					Installer::form_break();
					Installer::form_label("Addon");
					Installer::form_checkbox(["id"=>"php_memcache",     "label"=>"memcache",     "checked"=>true]);
					Installer::form_checkbox(["id"=>"php_mongodb",      "label"=>"mongodb",      "checked"=>true]);
					Installer::form_checkbox(["id"=>"php_redis",        "label"=>"redis",        "checked"=>true]);
	...
	public function install()
	{
		...
		$ext_addon  = ';extension=php_memcache.dll'.N;
		$ext_addon .= ';extension=php_mongodb.dll'.N;
		$ext_addon .= ';extension=php_redis.dll'.N;

		# for purposes of the loop, pretend this is a normal extension for now, will change in a few lines
		$ext_addon .= ';extension=php_xdebug.dll'.N;

		$data = str_replace(';extension=php_xsl.dll',        ';extension=php_xsl.dll'.N.$ext_addon,							$data);

		foreach($_SESSION['pdata'] as $key => $val)
		{
			if (substr($key, 0, 4) == 'php_')
				$data = str_replace(';extension='.$key.'.dll', 'extension='.$key.'.dll', $data);
		}
		...
		$data = str_replace('extension=php_xdebug.dll', 'zend_extension=php_xdebug.dll',                          			$data);
